...
export default function CanvasPage() {
  ...
  const addWatermarkAndDownload = (imageUrl: string) => {
    ...
    image.onload = () => {
      ...
      const fontSize = Math.max(12, image.width * 0.015);
    ...
    image.onerror = () => {
      toast({
        ...
        description: 'Could not load the image to add watermark.',
      });
    }
  };
  ...
  const handleDownload = useCallback(async () => {
    ...
    try {
      ...
      addWatermarkAndDownload(result.enhancedImageDataUri);
      ...
      addWatermarkAndDownload(dataUri);
    } finally {
      setIsProcessing(false);
    }
  }, [getCanvasData, toast]);
