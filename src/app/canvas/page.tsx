...
export default function CanvasPage() {
  ...
  const addWatermarkAndDownload = (canvas: HTMLCanvasElement) => {
    ...
    // Apply a sharpening filter for clarity
    tempCtx.filter = 'contrast(110%) saturate(110%)';
    tempCtx.imageSmoothingEnabled = false;

    // Draw the original canvas content upscaled
    tempCtx.drawImage(canvas, 0, 0, tempCanvas.width, tempCanvas.height);
    
    // Reset filter before drawing text
    tempCtx.filter = 'none';

    // Add watermark
    const fontSize = Math.max(12, tempCanvas.width * 0.015);
    tempCtx.font = `italic ${fontSize}px Poppins, sans-serif`;
    tempCtx.fillStyle = 'rgba(0, 0, 0, 0.6)';
    tempCtx.textAlign = 'right';
    tempCtx.textBaseline = 'bottom';
    tempCtx.fillText('Generated by Sasha', tempCanvas.width - 10, tempCanvas.height - 10);
  ...
  const handleDownload = useCallback(async () => {
    const canvas = canvasRef.current;
    if (!canvas || isCanvasEmpty.current) {
        toast({
            variant: 'destructive',
            title: 'Canvas is empty',
    ...
    toast({
      ...
      description: 'Enhancing image clarity and resolution.',
    });

    // Use a timeout to allow the toast to render before the blocking canvas operations
    setTimeout(() => {
      try {
        addWatermarkAndDownload(canvas);
      } catch (error) {
         console.error('Error enhancing image for download:', error);
         toast({
            variant: 'destructive',
